...
dates, highs, lows = [], [], []  # an empty list is formed called highs
for row in reader:
    current_date = datetime.strptime(row[2], '%Y-%m-%d')  # Corrected date format
    try:
        high = int(row[3])  # showing the position of the data in the row
        low = int(row[4])
    except ValueError:
